...
class TableInform:
    ...
    def __create_folder(self, oj_name):
        ...
        #     question_folder_name = oj_algorithms
        #     if os.name != 'posix':
    ...
    def update_table(self, oj):
        ...
        for _, _, files in os.walk(oj_algorithms):
            # print(files)
            for item in files:
                # print(os.path.abspath(item))
                # print(folder)
                qid =str(item.split('-')[0])
                completed = 0
                ...
                if item.endswith('.py'):
                    ...
                    # folder_url = 
                    folder_url = '/'.join([Config.github_leetcode_url, item])
                    # print(folder_url)
                    self.table_item[qid].python = '[Python]({})'.format(folder_url)
                    ...
                    # folder_url = os.path.join(folder_url, item)
                    folder_url = '/'.join([Config.github_leetcode_url, item])
                    self.table_item[qid].java = '[Java]({})'.format(folder_url)
                    ...
                    folder_url = '/'.join([Config.github_leetcode_url, item])
                    # print(folder_url)
                    self.table_item[qid].c_plus_plus = '[C++]({})'.format(folder_url)
