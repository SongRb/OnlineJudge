...
class Downloader:
    ...
    def login(self):
        loginCSRF = requests.request('GET', constants.LOGIN_URL).cookies.get_dict()['csrftoken']
    ...
    def download(self):
        ...
        for problem_set in constants.PROBLEM_SETS:
            response = self.session.get(
                constants.PROBLEMS_URL + problem_set,
    ...
    def save_solutions(self, problem_set, questions):
        print('Accepted question count: ', len(questions))
    ...
    def save_solution_file(self, problem_set, file_name, code_type, code):
        ...
        filepath = '{0}/{1}-{2}.{3}'.format(self.algorithms_path, self.title2id[file_name],
                                            file_name, code_type)
